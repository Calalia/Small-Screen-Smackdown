...
public class PC_LIIKKUU_10 : MonoBehaviourPunCallbacks
{
    ...
    public float cameraRotation=45f;
    ...
    void Start()
    {
        if (PhotonNetwork.IsMasterClient) cameraRotation = 225f;
        //Special Skills
    ...
    void FixedUpdate()
    {
        ...
        if ((photonView.IsMine == false && PhotonNetwork.IsConnected == true) || GetComponent<Healthbar>().dead)
        {
            return;
        }
        ...
        input = Quaternion.AngleAxis(cameraRotation,Vector3.up) * input;
        float moveHorizontal = input.x;
        float moveVertical = input.z;
    ...
    public void Attack()
    {
        ...
        if (canAttack && currentTarget != null && !GetComponent<Healthbar>().stunned)
        {
            ...
            GameManager.instance.attackCD.GetComponent<Cooldownimage>().StartCooldown(attackSpeed);   
    ...
    public void Dash()
    {
        if(!dashCoolDown)
        {
            ...
            dashtime = 0f;
            Invoke("DashCD", 8f);
            GameManager.instance.dashCD.GetComponent<Cooldownimage>().StartCooldown(8f);
        }
    ...
    {
        if (GameManager.instance.specialCD.GetComponent<Cooldownimage>().Unlocked == false) return;
        switch (faction)
        {
            case 0://Skeleton
                {
                    ...
                    specialCoolDown = true;
                        ...
                        if (skeletonSacrifice.GetComponent<Healthbar>().side == gameObject.GetComponent<Healthbar>().side)
                        {
                            ...
                            Invoke("SpecialCD", 15);
                            GameManager.instance.specialCD.GetComponent<Cooldownimage>().StartCooldown(15f);
    ...
    void LevelUp()
    {
        switch (level)
        {
            ...
            case 1:
                ...
                GameManager.instance.specialCD.GetComponent<Cooldownimage>().Unlocked = true;
                if (faction == 0)
                {
                    skeletonBonusToDamage = 3;
    ...
    public void GainXp(int xpAmount)
    {
        ...
        if (GetComponent<Healthbar>().dead != true)
        {
            xp += xpAmount;
        }
        #region XP check and updating Xp frame;
        //Checks if Player has reached the exp required to level up.
        if (xp >= xpRequired)
        {
            LevelUp();
            GameManager.instance.levelBar.GetComponent<Image>().sprite = GameManager.instance.levelSprites[level - 1];
        }
        GameManager.instance.levelFill.GetComponent<Image>().fillAmount =(float) xp / xpRequired;
